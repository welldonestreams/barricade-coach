...
def examples(paths,split='train'):
    ...
    for path in paths:
        path=Path(path)
        if path.suffix=='.json':
            try:loaded_rows=json.loads(path.read_text(encoding='utf-8'))
            except (OSError,json.JSONDecodeError):continue
            if not isinstance(loaded_rows,list):continue
            sources=[]
            for case in loaded_rows:
    ...
    for epoch in range(epochs):
        ...
        for row in rows:
            ...
            for move,p in probs.items():
                delta=weight*(p-target.get(move,0.0))
                for key in pv.policy_keys(g,move):policy[key]=policy.get(key,0.0)-step*delta
            policy_loss-=weight*sum(t*math.log(max(1e-12,probs.get(m,1e-12))) for m,t in target.items())
            ...
            if wanted is not None:
                ...
                grad=weight*(pred-wanted)*(1-pred*pred)
                for key in pv.state_features(g):value[key]=value.get(key,0.0)-value_rate*grad
                value_loss+=weight*(pred-wanted)**2
            count+=1;total_weight+=weight
        print(json.dumps(dict(epoch=epoch+1,examples=count,
              policy_loss=policy_loss/max(1,total_weight),value_loss=value_loss/max(1,total_weight))),flush=True)
