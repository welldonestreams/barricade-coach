...
def main():
    ...
    ap.add_argument('--accounts',default='steak2222');ap.add_argument('--prefix',default='steak')
    ...
    try:
        ...
        run.step('frozen-selfplay',['league_selfplay.py','--games',args.selfplay_games,
                 '--seconds','.18','--workers','4','--raw-only','--no-production-safety','--seed','20260910',
                 '--output',rel(baseline)])
        ...
        inputs=[teacher,late,baseline,verified,
        ...
        run.step('train-candidate',['train_nn.py',*[str(p) for p in inputs],
                 ...
                 '--feature-workers','8','--seed','20260910','--output',str(candidate)])
        ...
        if report.get('promoted'):
            run.step('post-promotion-league',['league_selfplay.py','--games','200','--seconds','.18',
                     '--workers','4','--no-production-safety','--seed','20260911',
