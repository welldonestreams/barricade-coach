...
(() => {
  ...
  panel.innerHTML='<div style="display:flex;gap:6px;align-items:center"><strong>Coach 1.3</strong><button id="bc-red">Red</button><button id="bc-blue">Blue</button><button id="bc-on">Pause</button></div><div id="bc-move" style="font-size:21px;color:#6ee7b7;margin-top:8px">Reading board…</div><div id="bc-status" style="font-size:12px;margin:6px 0"></div><div id="bc-why" style="font-size:13px;line-height:1.5"></div><div id="bc-opp" style="font-size:12px;color:#cbd5e1;margin-top:8px"></div>';
  ...
  (async()=>{
    for(const port of [8810,8811]) {
      ...
      const compatible=await new Promise(resolve=>GM_xmlhttpRequest({method:'GET',url:base+'/api/health',timeout:1500,
        onload:r=>{try{const h=JSON.parse(r.responseText);resolve(h.service==='barricade-coach'&&h.live_protocol>=6)}catch{resolve(false)}},
